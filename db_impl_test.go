...
func TestDBImplBasicWriteReadV2(t *testing.T) {
	db := setupDB(t)
	defer teardownDB(db)
	...
	readVal, readMeta, err := db.GetV2(ns[:], key, &ReadOptions{})
	assert.NoError(t, err)
	assert.Equal(t, value, readVal)
	assert.Equal(t, meta, readMeta)

	// read without options
	readVal, readMeta, err = db.GetV2(ns[:], key, nil)
	assert.NoError(t, err)
	assert.Equal(t, value, readVal)
	assert.Equal(t, meta, readMeta)
}
...
func TestDBImplConcurrentWriteAndReadV2(t *testing.T) {
	db := setupDB(t)
	defer teardownDB(db)
	...
	for i := 0; i < 50; i++ {
		...
		go func(i int) {
			...
			for j := 0; j < 500; j++ {
				key := genBytes(i, j)
				val, _, err := db.GetV2(ns[:], key, &ReadOptions{})
				if err != nil {
					print(err.Error())
				}
				assert.Nil(t, err)
